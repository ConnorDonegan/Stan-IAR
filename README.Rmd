...
That function returns a list with everything the `BYM2.stan` model needs for the IAR component, but it sets the scale factor to 1. That's okay but won't produce the BYM2 model. Add in the scale factor: 
```{r}
## make scale_factors
...
And we can view a sample of the variety of spatial autocorrelation patterns that are present in the prior model for $\phi$:
```{r}
...
And summarize the degree of spatial autocorrelation in each posterior draw of $\phi$ with the Moran coefficient:
